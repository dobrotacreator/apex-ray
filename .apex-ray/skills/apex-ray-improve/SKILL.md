...
- If a comparable eval case is missing and the user asked for a fresh analysis, capture or replay narrowly with `uv run --locked apex-ray eval capture-prs --pr <number>` and `uv run --locked apex-ray eval run-prs` rather than running a broad historical benchmark.
