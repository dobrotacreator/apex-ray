...
The optional `review.gates.pre_push.fetch_base` policy resolves the selected base before any merge-base or diff calculation. Exact remote-tracking refs are fetched and canonicalized; short names are refreshed from `origin` only when an exact remote branch exists, then fall back to a locally resolvable commit-ish for stacked-branch workflows only after an authoritative not-found response. Unknown refs and operational remote lookup or fetch failures fail before review. The policy is disabled by default, so ordinary review never introduces network access.
